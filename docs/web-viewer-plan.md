...
> **상태: 구현 완료 (계획 이력).** 확정된 설계는 `docs/architecture.md`의 "Web Viewer"
> 절로 이관했고, 사용법은 `README.md`에 있다. 이 문서는 **왜 그렇게 갔는지**의 이력으로만
> 남긴다 — 현재 코드의 기준 문서가 아니다.
>
> 계획과 달라진 점:
> - **shadcn/ui 미채택.** 이 문서 자체가 기본 톤을 TUI 밀도로 재조정해야 한다고 적었는데,
>   실제 UI가 커스텀 고밀도 패널이라 덮어쓸 것이 쌓을 것보다 많았다. Tailwind가 토큰을
>   직접 들고 있다.
> - **2단계 "연결 수명"은 미러를 건드리지 않았다.** `SseStream`이 자기 헤드를 쓰고 소켓을
>   소유하는 구조로 해소돼, 소비자 없는 상태에서 미러 응답 경로를 고칠 이유가 없었다.
> - **경로 검증 위치.** 라우트별이 아니라 `with_repo` 한 곳. 라우트별로 두었더니 `/api/diff`가
>   traversal을 통과시켰다(8절 참고).
...
- **구현 중 드러난 것**: 계획은 경로 검증을 "tree/file/commit 전 엔드포인트가 공유"라고만
  적었는데, 그것을 라우트별로 구현하면 새 라우트가 빠뜨린다. 실제로 `/api/diff`가
  `../../etc/passwd`를 받아들였다 — `load_file_diff`는 경로를 파일이 아니라 git pathspec으로
  쓰므로 검증기에 닿지 않고, 빈 hunk와 함께 공격자 경로를 되돌려줬다. 검증을 dispatch 한
  지점(`with_repo`)으로 올려야 "어떤 로더를 부르느냐"와 무관하게 안전해진다.
...
- ~~**뷰어 인증 부트스트랩**~~ — 확정: 자체 생성. 포트·쿠키가 이미 분리돼 있어 자격까지
  분리하지 않으면 분리가 형식적이 된다.
