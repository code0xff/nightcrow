...
- 숨은 프로젝트의 terminal attention은 탭 이름 앞에 `•` 한 셀로 집계한다. 밝음/어두움만 500ms마다
  바꿔 점멸하므로 텍스트 폭과 mouse hit box는 움직이지 않는다. 프로젝트가 활성화되어 한 frame의
  terminal event를 소비하면 그 클라이언트에서만 읽음 처리한다.
