...
`POST /api/file`은 working-tree 파일을 쓰는 유일한 route다(`POST /api/paste-image`는 worktree 밖에만 쓴다). 다른 mutation과 같은 Origin 검사와 `SameSite=Strict` cookie로 CSRF를 막고, `/api/file` read가 쓰는 worktree gate(`resolve_in_workdir`: traversal·symlink·git 디렉터리 거부)를 그대로 통과한다. authenticated user는 terminal에서 이미 파일을 쓸 수 있으므로 같은 신뢰 경계 안이고, 존재하는 working-tree 파일만 대상이다(commit 버전은 read-only history). optimistic concurrency: body의 `base_hash`(편집이 시작된 blob oid)가 디스크의 현재 oid와 다르면 `409`로 `currentHash`와 함께 거부해 밑에서 바뀐 변경을 덮지 않으며, `force`가 이를 무릅쓴다. 성공 응답은 저장된 내용의 blob oid를 돌려주어 client가 다음 저장의 base로 삼는다.

`POST /api/paste-image`는 pane에 붙여넣은 이미지를 받는다. pane은 이 머신의 PTY이므로 그 안의 프로그램에게 이미지를 건네는 방법은 열 수 있는 파일뿐인데, 클립보드를 쥔 브라우저는 다른 머신일 때가 많다 — CLI 자신의 클립보드 붙여넣기가 닿지 못하는 이유가 그것이다. 그래서 바이트가 wire로 오고 서버가 파일로 쓴 뒤, client가 그 경로를 pane에 입력한다. body는 base64가 아니라 raw bytes이고, `conn::read_request`가 body를 문자열이 아닌 바이트로 돌려주는 것이 이를 가능하게 한다.

경로의 어느 부분도 caller에게서 오지 않는다. 이름은 난수이고 확장자는 magic byte로 판별하며(PNG/JPEG/GIF/WebP 외에는 `415`) 디렉터리는 `~/.nightcrow/tmp`로 고정이므로, 이 route는 caller가 고른 경로로 유도될 수 없고 worktree gate가 필요 없다. 저장소 **밖**에 쓰는 것은 의도다: 붙여넣은 스크린샷은 프로젝트의 변경이 아니므로 status에 나타나서는 안 된다. 오래된 파일은 write마다 나이(24h)와 개수로 정리한다.

request body 상한은 route마다 다르다. 대부분은 작은 control payload이므로 `MAX_BODY_BYTES`(64KB)이고, 문서 하나를 통째로 싣는 `POST /api/file`은 `MAX_FILE_WRITE_BYTES`(1MB), 스크린샷 하나를 싣는 `POST /api/paste-image`는 `MAX_PASTE_IMAGE_BYTES`(10MB)다. 상한을 넘는 body는 **읽지 않고 `413`으로 거부한다** — 잘라서 받으면 그 조각이 그대로 파싱될 수 있고, handler가 반쪽짜리 내용을 온전한 파일인 양 디스크에 쓰게 된다. 서버는 답하고 바로 닫으므로, 업로드 중이던 client는 응답 대신 끊긴 연결을 볼 수 있다.
...
pane에 붙여넣거나 떨어뜨린 이미지는 panel 하나에 건 capture 단계 listener가 가로챈다. 어느 pane인지는 event가 온 cell(`data-pane-id`)에서 읽으므로, paste는 포커스된 terminal이고 drop은 커서 아래 cell이다. 텍스트가 함께 실린 클립보드는 텍스트 붙여넣기로 둔다 — rich editor에서 복사하면 둘 다 실리고, 복사한 사람이 원한 것은 글자다. 이미지가 아닌 모든 것은 xterm 자신의 경로로 흘려보낸다.

편집 세션은 `hooks/useHtmlEditor.ts`가 들고 `components/content/HtmlEditor.tsx`가 그린다. 파일 패널의 연필 토글은 **working tree의 HTML 파일에만** 뜬다 — 커밋의 사본은 히스토리라 제자리 수정 대상이 아니다. 세션은 파싱한 소스 문자열과 그에 대한 patch 목록을 ref로 들고(키 입력이 프레임을 리렌더해 열린 편집을 무너뜨리지 않도록), 저장 시 원본+patch로 결과를 새로 만들어 `POST /api/file`에 보낸다. 렌더 후 검증(agent의 `ready` → `applyLiveLocks` → `locked` 회신)이 끝나기 전에는 편집이 열리지 않으며, 잠긴 블록 클릭은 편집 대신 이유를 보여준다. 툴바 저장은 프레임에 `flush`를 보내 열려 있는 편집을 먼저 커밋시키고(프레임 안 Ctrl+S는 agent가 커밋 후 `save`를 보내므로 같은 채널 순서로 이미 반영됨), 디스크가 앞서 있으면 편집을 쥔 채 덮어쓸지 묻는다. 프레임은 일반 프리뷰와 같은 sandbox(절대 `allow-same-origin` 없음)이고 네트워크를 직접 만지지 않는다 — 저장은 인증된 부모만 한다. 편집 엔진 청크는 lazy load라 읽기만 하는 세션은 비용을 치르지 않는다.
