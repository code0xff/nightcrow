...
- **diff와 파일 내용은 한 페인의 두 얼굴이다**(`types.ts`의 `FileSource`, `usePaneOpeners::showOtherFace`).
  ...
  **제약을 조건문이 아니라 데이터로 표현**해서, 눌러봐야 아무 일도 안 일어나는 버튼이 없다.
  ...
  파일 맨 위로 가면 아무 데도 안 간 것과 같다. TUI는 스크롤 위치로 hunk를 고르지만 브라우저 diff는
  커서가 없는 스크롤 컨테이너라 DOM 측정이 필요해, 첫 hunk의 new-side 첫 줄로 잡는다(스크롤하지
  않은 사람에게는 같은 답이다). 여러 파일에 걸친 diff와 통째로 삭제된 파일에는 답이 없다 — 줄 번호가
  어느 파일 것인지 알 수 없거나 new side가 아예 없다. **렌더가 아니라 raw로 연다**: "이 변경 주변을
