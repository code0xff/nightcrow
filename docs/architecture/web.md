...
request body 상한은 route마다 다르다. 대부분은 작은 control payload이므로 `MAX_BODY_BYTES`(64KB)이고, 문서 하나를 통째로 싣는 `POST /api/file`만 `MAX_FILE_WRITE_BYTES`(1MB)다. 상한을 넘는 body는 **읽지 않고 `413`으로 거부한다** — 잘라서 받으면 그 조각이 그대로 파싱될 수 있고, handler가 반쪽짜리 내용을 온전한 파일인 양 디스크에 쓰게 된다. 서버는 답하고 바로 닫으므로, 업로드 중이던 client는 응답 대신 끊긴 연결을 볼 수 있다.
