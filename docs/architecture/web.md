...
- **프로젝트 탭 라벨은 14자에서 잘리고 `…`가 붙는다**(`lib/tabLabel.ts`). 상한은 TUI의
  `TAB_TITLE_MAX_CHARS`와 같은 값이고 자르는 규칙도 같다 — 같은 프로젝트가 두 화면에서 다른
  이름으로 불리지 않게 하는 것이 상한을 공유하는 이유다. **너비를 통일하지는 않는다**: VS Code의
  `tabSizing`으로 치면 기본값인 `fit`에 라벨 상한만 얹은 것이고, `fixed`(전 탭 동일 폭)는 취하지
  않았다. 탭이 파일이 아니라 프로젝트라 많아야 열 개인데, 헤더 가로 공간은 open·Cloning·accent·
  reload·logout과 다투므로 짧은 이름에 빈 칸을 채워 넣을 여유가 없다. **자르는 곳은 서버가 아니라
  표시하는 쪽이다** — `RepoInfo.name`은 프로젝트 메뉴와 `aria-label`이 온전히 읽어야 하는 값이고,
  자르기는 탭 행에서만 참인 제약이다. 글자 수로 세고 표시 폭으로 세지 않는 것도 TUI와 같아서,
  한글 이름은 양쪽 모두에서 ASCII의 두 배쯤 차지한다.
...
- **터치 기기 터미널에는 소프트키보드가 못 내는 키를 얹는다**(`lib/termKeys.ts`, `Terminal.tsx`). `Ctrl-C`
  ...
  **원시 바이트**로 매핑해(`termKeySequence`: Esc=`\x1b`, `^C`=`\x03`, ↑=`\x1b[A`, ⇧Tab=`\x1b[Z` —
  ...
  닫히지 않게). 자주 쓰는 조합(`^B`/`^C`/`^D`/`^Z`/`^L`/`^R`)은 개별 버튼으로 둔다 — `^B`는 tmux
  prefix라 없으면 tmux 세션 안에서 아무것도 못 한다. 바에는 소프트키보드가 **못 내는** 키만 담는다.
  터치 기기에선 xterm 폰트도 한 포인트 키운다(`pointer: coarse`, 12→13px).
- **Ctrl은 개별 버튼으로 다 못 담으므로 sticky 래치를 하나 둔다**(`ctrlSequence`, `useCtrlLatch`).
  셸이 쓰는 조합은 바에 들어가는 대여섯 개보다 많다(`^A`/`^E`/`^K`/`^W`/`^P`…). Ctrl 버튼은 눌리면
  **무장**하고, 다음 글자가 제어바이트로 바뀌어 나간다. 가로채는 지점은 keydown이 아니라
  **`term.onData`**다: 소프트키보드·IME는 키를 설명할 의무가 없어(iOS·Android가 `Unidentified`,
  keyCode 229를 준다) keydown으로는 무엇이 눌렸는지 못 읽지만, onData에 도착한 시점에는 무엇이
  만들었든 글자다. 그래서 sticky를 접었던 이전 판단(“다음 입력을 가로채야 해 복잡하다”)이 뒤집힌다 —
  가로챌 자리가 이미 있고 필요한 것은 ref 하나와 순수 함수 둘이다. 제어형이 없는 입력(한글, 이모지,
  두 글자 이상)은 **그대로 흘리되 래치는 쓴다** — 사람이 친 것은 맞으니 소진되고, 삼키면 친 것을
  잃는다. 대문자화는 `toUpperCase`가 아니라 코드포인트로 한다(`ß`→`SS`처럼 두 글자가 되는 letter의
  앞 글자로 제어바이트를 만들지 않게).
  - **무장은 pane에 포커스를 준다**(`onArm`). 나머지 바 키는 소켓으로 바로 나가 포커스가 필요 없지만,
    래치를 쓰는 건 *친 글자*라 칠 곳이 없으면 아무 일도 안 일어난다.
  - **escape로 시작하는 입력은 래치를 쓰지 않는다**(`ctrlLatchStep`). 그 입력은 사람이 아니라 pane의
    프로그램이 보낸다 — tmux·vim이 켜는 focus 리포트(`ESC [ I`), 마우스 리포트. 무장이 포커스를 주는
    순간 그게 제일 먼저 도착하므로, 소진 대상에 넣으면 래치는 한 글자도 못 받고 죽는다. 두 리포트만
    골라내지 않고 **prefix 전체**로 잡는 것은, 터미널의 자동 응답이 전부 escape로 시작하기 때문이다
    (커서 위치 응답, device attributes…) — 목록에서 하나 빠질 때마다 래치가 소리 없이 죽는다. 대가는
    반대편 오탐이다: bracketed paste(`ESC [ 2 0 0 ~`)와 하드웨어 Esc·화살표는 래치를 안 쓴다. 틀리려면
    이쪽으로 틀리는 게 낫다 — 버튼이 켜진 채라 상태가 화면에 있고 한 번 눌러 끄면 되지만, 죽은 래치는
    안 바뀐 글자로만 발견된다.
  - **바가 사라지면 래치도 푼다**(`TermKeyBar`의 unmount). 래치는 패널 소유라 바보다 오래 사는데,
    무장 상태를 말해주는 버튼이 없는 채로 남으면 한참 뒤 글자를 이유 없이 바꾼다. 툴바에서 감추거나
    마지막 pane이 닫히면 unmount다. 폰에서 다른 뷰를 골라 패널이 `display:none`인 동안은 바도 래치도
    그대로 살아 있는데, 그건 의도다 — 켜진 채로 함께 돌아온다. 다른 바 키를 눌러도 풀린다: 그 키들은
    이미 자기 바이트를 갖고 있어 Ctrl이 붙을 자리가 없다.
  - **경계**: bracket 없는 한 글자짜리 붙여넣기는 그 글자를 친 것과 구분되지 않는다(xterm이 같은
    데이터로 준다). 무장 중이면 제어바이트가 된다. replay 중처럼 `active`가 아직 null이면 무장이
    포커스를 줄 곳이 없다 — 래치는 남아 있다가 첫 글자에 붙으므로 잃는 것은 없다.
  - 무장 상태는 버튼이 켜져 보이는 것으로만 알 수 있으므로 `aria-pressed`와 accent 색을 함께 준다.
    래치가 만든 `0x0c`는 `ClearKeyProbe`에 보고하지 않는다 — 프로브는 clear 바이트를 그 뒤의 keydown과
    짝지어 “키 없이 도착한 것”을 찾는 장치이고(`lib/clearKeyProbe.ts`), 바가 만든 바이트는 그 찾는
    모양과 똑같이 기록되어 오히려 수사를 흐린다. 바의 `^L` 버튼도 같은 이유로 이미 프로브 바깥이다.
...
- **패널이 비면 키보드를 `+`로 넘긴다**(`focusFillsEmptyPanel`, `PanelToolbar`). 마지막 pane이 나가면
  키보드가 있었을 만한 것이 전부 함께 사라진다 — 그 pane의 xterm, 아래 키 바, 키 바 토글. 렌더가
  끊긴 엘리먼트는 body로 blur되고 body에서 Tab은 페이지 맨 위부터 다시 센다. 그래서 **비는 순간
  하나 남은 할 일**인 `+`에 얹는다. **body일 때만** 한다 — 다른 데 있는 포커스는 이 일로 잃은 게
  아니고, 가져오면 남의 타이핑을 끊는 것이다(`focusIsTakeable`보다 좁다). 사라진 엘리먼트가 무엇이었는지
  body는 말해주지 않으므로 pane 개수의 **엣지**로 판단한다. **엣지이므로 경계가 둘 있다**: 패널이
  `display:none`인 동안(폰에서 다른 뷰를 고른 상태) 마지막 pane이 나가면 숨은 버튼에 건 `focus()`는
  아무 일도 하지 않고, 나중에 터미널 뷰로 돌아와도 개수는 계속 0이라 다시 시도하지 않는다 — 그때
  포커스는 애초에 이 패널에 있지 않았다. 그리고 body를 "이 일로 잃은 포커스"로 읽으므로, 스크롤 중이라
  마침 body였던 화면에서 재연결이 pane 목록을 비우면 `+`가 가져간다 — 아무도 안 쥔 키보드를 가져가는
  것이라 패널이 이미 쓰는 정책(`focusIsTakeable`)과 같은 판단이다.
