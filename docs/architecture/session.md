...
- **에뮬레이터가 셀을 읽으므로 resize를 따라가야 한다**(`hub_layout::resize_pane`). 그리드가 틀린
  ...
  그리드가 파서의 스크래치였고 resize를 따르지 않았다.
- **화면 하나로는 부족하다: `screen` + `since`.** 스냅샷은 worker tick마다 갱신되므로 chunk가
  ...
  바이트는 건너뛸 수 없다). 상한을 넘으면 그 자리에서 새 스냅샷을 떠서 비운다. 버퍼를 전환하는
  chunk는 tick을 기다리지 않고 즉시 스냅샷한다 — 전환 전 화면 위에 전환 후 바이트가 얹히는 창을
  남기지 않기 위해서다.
- **alternate screen 동안 링은 동결된다.** 그 바이트는 링에 쓸모없고, 프로그램이 전환한 시점의
  링이 곧 그가 돌아갈 normal screen이다. 그래서 alt pane의 replay는 `1049l` + 링(normal screen)
  → prelude(`1049h`) → `screen` + `since` 순서다. 이것이 없을 때는 full-screen 프로그램을
  종료하면 그 도중에 붙은 클라이언트가 빈 화면을 봤다. 남은 한계: 전환이 일어난 **그 chunk**의
  전환 이전 텍스트는 링에 들어가지 않는다(chunk 단위로 기록하므로).
...
- **스냅샷이 나르지 않는 것**: wrap 기록(`WRAPLINE`, 마지막 칼럼에서 밀린 wide char의
  ...
  다시 그리고, normal pane은 링으로 replay되어 wrap이 보존된다). underline 색, 하이퍼링크(OSC 8),
  스크롤 리전(DECSTBM)도 나르지 않는다.
