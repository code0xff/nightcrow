...
> **상태: 진행 중** — A·C·D·F 완료, E 진행 중(터미널 중계와 비동기 pane 생성까지). 확정된 설계는 단계가 끝나는 대로
...
1. **`HubBackend`** — `TerminalBackend`를 데몬 소켓 위에 얹는다. 저장소당 하나이고,
   `DaemonClient`의 연결을 공유한다. `DaemonClient`가 수신 스레드에서 저장소별 큐로
   라우팅하고(`Arc<Mutex<HashMap<repo, VecDeque<BackendEvent>>>>`), 각 `HubBackend`가
   자기 큐만 드레인한다. 쓰기는 `Arc<Mutex<UnixStream>>` 공유.
2. **`requested`를 어떻게 아는가** — 지금 허브의 `Created`에는 누가 요청했는지가 없다.
   확정: 허브 `ServerMessage::Created`에 `client: u64`를 싣고(허브는 이미
   `Command::Create { client }`로 알고 있다), 데몬이 hello 응답에 **그 클라이언트의 id**를
   실어주며, 클라이언트가 비교한다. 브라우저는 모르는 필드를 무시하므로 웹은 무영향.
3. **크기 소유권** (6절) — pane 크기는 소유자 클라이언트가 정한다. attach 시 자동 이동,
   키로 명시적 탈취. 비소유 클라이언트는 resize를 보내지 않는다.
4. **`ui/terminal_tab/`이 할당 영역보다 작은 그리드**를 그리는 경우. 지금은 항상
   area == PTY 크기라 이 경우를 모른다.
5. **startup command를 데몬이 한 번만 실행**한다(hub의 `claim_startup` 재사용).
   `ensure_initial_terminal`은 로컬 백엔드 전용 경로가 된다.
6. **스크롤백 깊이 실측** (10절 1번) — 허브는 바이트 링버퍼, TUI는 1000줄 기준이다.
