...
From `md` width up, the project tabs run across the header or down the page's left edge; the header button beside the accent swatch moves them, and the TUI's `[layout] tabs = "left"` is the same idea for a terminal. On the left they stand beside the whole page, header included, one project per row, with the same labels, drag-to-reorder and close control as the row. Like the pane view mode, the choice is this browser's own — a screen with rows to spare is not the screen with columns to spare — and it is stored in the browser, not in `viewer.json`. Below `md` there is no strip either way; the header's project menu takes its place.
