...
- **트리 캐시는 저장소를 함께 들고 다닌다**(`viewer-ui/src/lib/treeCache.ts`). 디렉토리 목록의 키는 저장소 상대 경로(`src`, `src/lib`)라 **어느 프로젝트 것인지가 키에 없다.** 프로젝트 두 개가 같은 디렉토리 이름을 가지면 키가 겹치고, 경로만 기억하는 캐시는 한쪽 목록을 다른 쪽에 내준다. 게다가 트리는 **이미 가진 경로는 다시 읽지 않으므로**(`toggleTreeDir`/`revealTreeDir`) 잠깐 스쳐가는 오류가 아니라 새로고침 전까지 고정된다 — `Sidebar`는 프로젝트를 바꿔도 remount되지 않아서 `useTree`의 상태가 그대로 살아남는다. 그래서 캐시가 `repo`를 자기 안에 들고, 읽기는 전부 `forRepo`를 거치며(저장소가 다르면 빈 캐시), 쓰기는 `withChildren(cache, repo, …)`로 요청 당시의 저장소를 함께 받는다 — 전환 전에 보낸 응답이 늦게 도착해도 현재 프로젝트 캐시에 앉지 못한다. status(`setStatus(null)`)와 log(`resetLog`)는 이미 저장소 변경에서 초기화되고 있었고, 트리만 빠져 있었다.
