...
`objects`/`logs`/`*.lock`을 거르는 규칙은 **그 경로가 실제로 속한 git 디렉토리 기준**으로 본다.
서브모듈은 `modules/<name>/` 아래에 자기 git 디렉토리를 갖고 거기서도 같은 churn이 나므로, 최상위
기준으로만 보면 서브모듈 fetch가 만드는 오브젝트마다 부모 트리를 걷게 된다. `modules` 뒤에 이름이
따라올 때만 벗겨서, `objects`라는 이름의 서브모듈이 있어도 그 index는 오브젝트 디렉토리로 오해되지
않는다.
