...
- **트리 캐시는 저장소를 함께 들고 다닌다**(`viewer-ui/src/lib/treeCache.ts`). 디렉토리 목록의 키는 저장소 상대 경로(`src`, `src/lib`)라 **어느 프로젝트 것인지가 키에 없다.** 프로젝트 두 개가 같은 디렉토리 이름을 가지면 키가 겹치고, 경로만 기억하는 캐시는 한쪽 목록을 다른 쪽에 내준다. 게다가 트리는 **이미 가진 경로는 다시 읽지 않으므로**(`toggleTreeDir`/`revealTreeDir`) 잠깐 스쳐가는 오류가 아니라 새로고침 전까지 고정된다 — `Sidebar`는 프로젝트를 바꿔도 remount되지 않아서 `useTree`의 상태가 그대로 살아남는다. 그래서 캐시가 **어느 방문(`TreeOwner`) 것인지**를 자기 안에 들고 다닌다. 읽기는 `visibleFor(cache, repo)`를 거친다 — 저장소 이름이 화면과 다르면 목록이 아니라 빈 캐시가 나오므로 **한 프레임도 남의 파일을 그리지 않는다**. 쓰기는 `withChildren(cache, current, requested, …)`로 **화면의 방문과 요청 당시의 방문을 둘 다** 받고, 둘을 **이름이 아니라 객체 identity로** 비교한다: 목록은 자기를 부른 클릭보다 오래 살아서 전환 전에 보낸 요청이 전환 후에 도착할 수 있고, `r1 → r2 → r1`처럼 되돌아오면 이름만으로는 옛 응답이 현재 것처럼 보인다. 저장소가 바뀔 때마다 새 방문 객체를 만들어 두면 그런 응답이 최신 목록을 덮지 못한다. 방문 객체는 **렌더가 아니라 effect에서** 갱신한다 — 렌더 중 ref 쓰기는 커밋되지 않은 렌더의 저장소를 남길 수 있다. 읽기가 identity가 아니라 이름을 보는 이유는, 전환 렌더와 방문을 여는 effect 사이 한 프레임 동안 캐시가 아직 직전 방문 소유라서 identity로 가리면 트리가 깜빡이기 때문이다. status(`setStatus(null)`)와 log(`resetLog`)는 이미 저장소 변경에서 초기화되고 있었고, 트리만 빠져 있었다. **파일명 검색 결과도 같은 이유로 태그를 단다**(`TreeMatches`, `matchesFor`) — 결과 역시 저장소 상대 경로이고, effect에서 비우면 새 프로젝트가 이전 결과를 그리는 프레임이 한 번 남는다. **터미널 소켓도 같은 계열이다**(`useTerminalSocket.ts`): pane id는 저장소 안에서만 의미가 있어서, 전환 순간에 이미 날아오던 프레임이 새 프로젝트의 같은 id pane에 붙을 수 있다. `onmessage`는 자기 소켓이 아직 살아 있는 소켓인지 먼저 확인한다. 여기서 정리는 **passive가 아니라 layout effect**다: 터미널 패널은 저장소별로 remount하지 않으므로(저장소별 마지막 포커스 pane을 기억해야 한다) 전환 렌더가 이전 프로젝트의 pane과 xterm DOM을 그대로 커밋한다. passive effect는 그게 페인트된 뒤에 돌 수 있어 이전 터미널이 한 프레임 보인다.
