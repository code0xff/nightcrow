...
위험(리눅스 inotify 디스크립터 소진)은 **설치 실패 시 예전의 1초 폴링으로 폴백**해서 받는다.
...
**macOS는 이벤트 경로를 심링크 해석해서 준다**(`/var/...` → `/private/var/...`). 그래서 워크트리
경로를 canonical 형태와 원래 형태 양쪽으로 들고 비교한다 — 상대 경로로 만들지 못한 이벤트는 "판단
불가 → 읽는다"로 떨어지므로, 이걸 틀리면 정확성은 유지되지만 **ignore 필터가 조용히 통째로
무력화된다**(안 만든 것과 같아진다).
