...
**아무도 안 보는 저장소는 걷지 않는다.** 데몬은 저장소마다 스냅샷 워커를 띄우지만, 구독자가
없으면 트리 순회를 멈춘다(`SnapshotChannel::watch`). 첫 구독자가 오면 다시 켜고 **그 자리에서 한 번
읽어** 답한다 — 꺼져 있는 동안의 `latest`는 마지막 클라이언트가 떠날 때의 상태이고, 다음 날 아침에 연
페이지에는 그것이 낡은 값이 아니라 틀린 화면이다. `/api/status`도 꺼져 있으면 같은 방식으로 읽는다.
남는 중복은 브라우저와 터미널이 동시에 볼 때인데, 그때는 양쪽이 정말로 그 데이터를 원한다.
