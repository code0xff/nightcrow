...
**세션** — 저장소별 파일이므로 탭을 닫을 때와 종료 시 각각 저장한다. 자기
복원이 아직 실행되지 않은 프로젝트는 live 절반(pane/focus/fullscreen)과
pending 절반(selection/scroll/tree)을 병합해 저장한다(`session_to_save`).
어느 쪽도 건너뛰거나 덮어쓰지 않으므로, 스냅샷 실패 경로가 둘 중 하나를
고를 필요가 없다.

**열린 탭 목록**은 `~/.nightcrow/workspace.json`에 따로 저장한다. 어떤 저장소
하나도 "옆에 다른 셋이 열려 있었다"는 사실을 소유하지 않으므로 repo 안이 아니라
config 옆에 둔다. `--repo`가 주어지면 복원하지 않는다 — 명시적 인자가 이긴다.
빈 목록도 기록한다: 탭을 다 닫고 종료하는 것이 다음 실행을 빈 화면으로
시작하는 방법이고, 기록을 건너뛰면 이전 탭이 되살아난다.
