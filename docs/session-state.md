...
- `viewer.json` stores the session accent and the browser layout preferences that mean the same thing on every screen: the upper-panel split, per-project view, and maximized panel (up to 50 recent projects). The sidebar width is a pixel count and so a fact about one screen; each browser keeps its own in `localStorage`, alongside the pane view mode, the key bar, and the shortcut leader. Browser terminal panes and their live arrangement end with the session.
