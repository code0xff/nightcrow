...
fn matcher_group() -> Value {
    ...
    group.insert(
        MATCHER_KEY.to_string(),
        Value::String(HOOK_MATCHER.to_string()),
    );
...
pub(crate) fn merge_into(settings: &mut Value, exe: &str) -> Result<(Vec<String>, Option<Value>)> {
    ...
    let command = hook_command(exe);
    {
        ...
        let groups = array_mut(groups, "`hooks.StopFailure`")?;
        let index = match groups
            .iter()
            .position(|g| g.get(MATCHER_KEY).and_then(Value::as_str) == Some(HOOK_MATCHER))
        {
            Some(index) => index,
            None => {
                groups.push(matcher_group());
                groups.len() - 1
            }
        };
        let group = object_mut(&mut groups[index], "the `rate_limit` matcher group")?;
        let entries = group.entry(HOOKS_KEY).or_insert_with(|| json!([]));
        let entries = array_mut(entries, "the `rate_limit` group's `hooks`")?;
        // An exact command match already present is what makes a second install
        // a no-op instead of a duplicate entry.
        let present = entries
            .iter()
            .any(|e| command_of(e) == Some(command.as_str()));
        if !present {
            entries.push(hook_entry(&command));
            changes.push(format!(
                "added {HOOK_EVENT} hook for matcher `{HOOK_MATCHER}`: {command}"
            ));
        }
    }
...
pub(crate) fn strip_from(settings: &mut Value, restore: Option<Value>) -> Result<Vec<String>> {
    ...
    if removed > 0 {
        changes.push(format!("removed {removed} {HOOK_EVENT} hook command(s)"));
    }
...
fn strip_hooks(root: &mut Map<String, Value>) -> usize {
    ...
    if let Some(hooks) = root.get_mut(HOOKS_KEY).and_then(Value::as_object_mut) {
        ...
        // `StopFailure` list survives an uninstall that found nothing of ours.
