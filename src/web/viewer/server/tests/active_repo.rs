...
pub(super) fn served_ids(addr: std::net::SocketAddr, token: &str) -> Vec<String> {
...
pub(super) fn served_active(addr: std::net::SocketAddr, token: &str) -> serde_json::Value {
    let list = get(addr, "/api/repos", Some(token));
    let value: serde_json::Value = serde_json::from_str(body_of(&list)).unwrap();
    value["active_repo"].clone()
}

pub(super) fn select(addr: std::net::SocketAddr, id: &str, token: &str) -> String {
    post(
        addr,
        "/api/prefs",
...
pub(super) fn server_at(prefs_dir: &std::path::Path, paths: &[String]) -> super::ViewerServer {
    server_with(
        paths,
