...
pub(super) fn seed_log_app(entries: usize, page_size: usize, threshold: usize) -> App {
    ...
    app.git.view.log.set_commits(commits);
    app
}
...
fn worker_reply_appends_matching_tail() {
    ...
    assert_eq!(app.git.view.log.loaded_count, 2);
}
...
fn worker_reply_discards_stale_tail() {
    ...
    app.flush_commit_log_fetch_for_test(Duration::from_secs(2));
    assert_eq!(app.git.view.log.commits.len(), 2);
    assert_eq!(app.git.view.log.commits[0].summary, "c9");
}
...
fn refresh_after_head_change_prepends_new_commit() {
    ...
    let prior_oid = app.git.view.log.commits.first().unwrap().oid;
    ...
    // looking at c0.
    assert_eq!(app.git.view.log.commits.len(), 2);
    assert_eq!(app.git.view.log.commits[1].oid, prior_oid);
...
fn refresh_after_head_change_keeps_merged_side_branch_commits() {
    ...
    assert_eq!(app.git.view.log.commits.len(), 2);
    assert_eq!(app.git.view.log.commits[0].summary, "c1");
    ...
    assert_eq!(app.git.view.log.commits.len(), 4);
    drop(dir);
}
...
fn refresh_after_head_change_resets_on_divergence() {
    ...
    assert_eq!(app.git.view.log.commits.len(), 1);
    assert_ne!(app.git.view.log.commits[0].oid, ghost_oid);
