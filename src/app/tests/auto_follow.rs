...
fn ingest_snapshot_populates_hot_table_from_mtimes() {
    ...
    assert_eq!(app.git.view.status.hot_table.len(), 2);
    assert!(app.git.view.status.hot_table.contains_key("a.rs"));
    assert!(app.git.view.status.hot_table.contains_key("b.rs"));
}
...
fn merge_hot_table_replaces_only_when_newer() {
    ...
    assert_eq!(app.git.view.status.hot_table.get("a.rs"), Some(&newer));
}
...
fn auto_follow_selects_freshest_hot_file_when_idle() {
    ...
    assert_eq!(
        app.git.view.auto_follow.followed_path.as_deref(),
        Some("b.rs")
    );
}
...
fn auto_follow_skipped_when_user_recently_navigated() {
    let mut app = app_with_files(vec!["a.rs", "b.rs"]);
    app.git.view.status.selected = 0;
    app.git.view.auto_follow.last_manual_nav_at = Some(Instant::now());
    let now = SystemTime::now();
    ...
    assert!(app.git.view.auto_follow.followed_path.is_none());
}
...
fn auto_follow_skipped_when_disabled_in_config() {
    let mut app = app_with_files(vec!["a.rs", "b.rs"]);
    app.git.agent_indicator.auto_follow = false;
    app.git.view.status.selected = 0;
    let now = SystemTime::now();
...
fn select_down_marks_user_active_when_focus_is_filelist() {
    ...
    app.git.view.auto_follow.followed_path = Some("a.rs".to_string());

    app.select_down();

    assert!(app.git.view.auto_follow.last_manual_nav_at.is_some());
    assert!(app.git.view.auto_follow.followed_path.is_none());
}
...
fn select_down_does_not_mark_when_focus_is_diff() {
    ...
    assert!(app.git.view.auto_follow.last_manual_nav_at.is_none());
}
...
fn auto_follow_respects_search_filter() {
    ...
    app.git.view.status.selected = 0; // alpha.rs (the only filtered entry)
    let now = SystemTime::now();
...
fn auto_follow_excludes_future_mtime_clock_skew() {
    ...
    assert_eq!(app.git.view.status.selected, real_idx);
    assert_eq!(
        app.git.view.auto_follow.followed_path.as_deref(),
        Some("real.rs")
    );
