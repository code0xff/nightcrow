...
impl App {
    pub(super) fn handle_commit_log_page_msg(&mut self, msg: CommitLogPageMsg) {
        if msg.generation != self.git.commit_log.generation() {
            return;
        }
        match msg.kind {
    ...
    fn apply_tail_page(&mut self, msg: CommitLogPageMsg) {
        ...
        if msg.skip != self.git.view.log.loaded_count {
            self.git.view.log.clear_pending();
            return;
        }
        match msg.result {
            Ok(page) => {
                self.git.view.log.append_page(page, msg.page_size);
    ...
    fn apply_refresh_page(
        ...
        let can_prepend = prepend_idx.is_some_and(|idx| {
            ...
            !self.git.view.log.commits.is_empty()
                && fresh_tail.len() <= self.git.view.log.commits.len()
                && fresh_tail
                    .iter()
                    .zip(self.git.view.log.commits.iter())
            ...
            && can_prepend
        {
            ...
            new_head_commits.append(&mut self.git.view.log.commits);
            self.git.view.log.commits = new_head_commits;
            self.git.view.log.loaded_count = self.git.view.log.commits.len();
            ...
            if page_is_short && self.git.view.log.commits.len() <= page_size {
                self.git.view.log.fully_loaded = true;
            }
            self.git.view.log.commit_width_cache.set(None);
            // Prepend bypasses `set_commits`, so refresh the filter cache
            // manually so an active search query resolves against the new head.
            self.git.view.log.recompute_commit_filter();
            self.git.view.log.clear_pending();
            ...
            {
                self.git.view.log.selected = pos;
            } else {
                ...
                self.git.view.log.selected = self
                    .git
                    .view
                    .log
                    .selected
            ...
            self.git
                .view
                .log
                .set_commits_from_first_page(page, page_size);
            self.git.view.log.selected = prior_selected_oid
                .and_then(|oid| self.git.view.log.commits.iter().position(|c| c.oid == oid))
                .unwrap_or(0);
        }
        ...
        if self.git.view.log.drill_down
            && prior_selected_oid
                .is_none_or(|oid| !self.git.view.log.commits.iter().any(|c| c.oid == oid))
        {
